  ...
  class PagelessRedirectGenerator < Generator
    ...
    def generate_aliases(destination_path, aliases)
      ...
      alias_paths.flatten.each do |alias_path|
        ...
        alias_index_path = alias_dir != '.' ? File.join(alias_dir, alias_file) : alias_file
        ...
        path = nil # accumulator for the directory structure
        alias_index_path.split('/').each do |part|
          path = path.nil? ? part : File.join(path, part)
          @site.static_files << Jekyll::PagelessRedirectFile.new(@site, @site.dest, path, '')
        end
      end
    end
